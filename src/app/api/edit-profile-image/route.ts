...
export async function PATCH(request: NextRequest): Promise<any> {
  ...
  try {
    ...
    if (user.coverImagePublicId) {
      const dcryptedPublicId = decrypt(user.coverImagePublicId);
    ...
    const updateProfile = await userModel.findByIdAndUpdate(
      ...
      {
        $set: {
          coverImage: fileResponse.secure_url || "",
          coverImagePublicId: encryptPublicId || "",
        },
      },
      { new: true }
    );
    ...
    return NextResponse.json(
      {
        success: true,
        message: "successfully updated cover iamge!!!",
        user: updatedUser,
      },
      { status: 200 }
    );
